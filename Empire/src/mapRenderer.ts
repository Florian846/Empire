...
export async function renderMap(
  ...
  try {
    const { textures, dirtSandBlendTextures, dirtStoneBlendTextures } =
    ...
    const sandGrassBlendMap = createBlendMap();
    const dirtStoneBlendMap = createBlendMap();
...
function setupDebugClickHandler(
  ...
  mapSprite.on("pointerdown", (event) => {
    ...
    if (getTerrainType(tileX, tileY) === "SAND") {
      ...
      console.log(
        `DEBUG: Clicked Sand Tile at [${tileX}, ${tileY}]. Mask Value: ${maskValue}`,
      );
    }
  });
}
